...
m115_asym_encode = function (src, srclen) {
    ...
    for (i = j = 0, ref = Math.floor((srclen + m - 1) / m);
        (0 <= ref ? j < ref : j > ref); i = 0 <= ref ? ++j : --j) {
        ret += srsa.encrypt(bytesToString(src.slice(i * m, Math.min((i + 1) * m, srclen))));
    }
    return window.btoa(srsa.hex2a(ret));
};
...
m115_asym_decode = function (src, srclen) {
    ...
    for (i = j = 0, ref = Math.floor((srclen + m - 1) / m);
        (0 <= ref ? j < ref : j > ref); i = 0 <= ref ? ++j : --j) {
        ret += srsa.decrypt(bytesToString(src.slice(i * m, Math.min((i + 1) * m, srclen))));
    }
